...
export default function Home() {
  ...
  // =========================================================================================
  // 🚀 NOVO: PREENCHEDOR DE IMAGENS HÍBRIDO (UNSPLASH E IA PAGA EM PARALELO)
  // =========================================================================================
  const preencherImagensAutomaticamente = async (htmlBruto: string, isFragment = false) => {
      ...
      try {
          ...
          const promessas = imagens.map(async (img) => {
              ...
              try {
                  // 1. HIDRATAÇÃO IA PAGA (Busca a foto direto no seu servidor sem timeout global)
                  if (srcOriginal && srcOriginal.includes('[IMAGEM_IA:')) {
                      const match = srcOriginal.match(/\[IMAGEM_IA:\s*([^\]]+)\]/);
                      if (match) {
                          const promptDaIa = match[1];
                          ...
                          if (res.ok) {
                              ...
                              if (data.url) {
                                  img.src = data.url;
                                  return; // Imagem gerada com sucesso, vai para a próxima.
                              }
                          }
                          // Se falhou (sem saldo ou erro do Google), põe o placeholder cinza
          ...
          // Roda todas as buscas ao mesmo tempo!
