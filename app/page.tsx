...
export default function Home() {
  ...
  const otimizarComIA = async (comandoOverride?: string) => {
      ...
      const systemInstruction = `Atue como Engenheiro de UI/UX e Copywriter Sênior. Receberá o HTML de UM elemento. Aplique a modificação: "${comando}". 
      REGRA MÁXIMA: DEVOLVA APENAS O HTML FINAL E OTIMIZADO. Não inclua explicações ou formatações extras. Preserve o ID original id="${elementoSelecionado.id}".`;
      ...
      if(resData && resData.html) {
          ...
          (window as any).showNotification("Elemento otimizado com sucesso.", "success");
      }
  };

  const chamarMotorIA = async (systemInstructionText: string, promptParts: any[], isElementRefinement = false) => {
    setStatusApis({ texto: isElementRefinement ? 'Processando Texto...' : 'Gerando Código da Estrutura...', processing: true });
  ...
  const getMegaPromptEstilo = () => {
    ...
    if (estilo === 'premium') return "DIRETRIZ VISUAL: Design sofisticado, aparência de alta qualidade. Tipografia elegante e espaçamentos precisos.";
    if (estilo === 'terapia') return "DIRETRIZ VISUAL: Layout minimalista, acolhedor. Muito espaço em branco e bordas suaves.";
    if (estilo === 'agressivo') return "DIRETRIZ VISUAL: Landing Page focada em alta conversão. Alto contraste, dark mode sugerido.";
    return "DIRETRIZ VISUAL: Interface moderna, limpa e focada em usabilidade profissional.";
  };

  const getMegaPromptCores = () => {
    const cor = (document.getElementById('paletaCores') as HTMLSelectElement)?.value || 'auto';
    if (cor === 'personalizada') return `CORES: Cor de Fundo: ${(document.getElementById('corFundo') as HTMLInputElement)?.value}, Cor de Destaque: ${(document.getElementById('corPrimaria') as HTMLInputElement)?.value}`;
    if (cor === 'auto') return "CORES: Extraia e aplique fielmente o esquema de cores da imagem fornecida.";
  ...
  const getMegaPromptHero = () => {
    ...
    if (hero === 'center') return "ESTRUTURA DO TOPO: Totalmente centralizado. Título, subtítulo e Botão alinhados ao meio.";
    if (hero === 'split') return "ESTRUTURA DO TOPO: Layout dividido. Texto persuasivo na esquerda, Imagem de impacto na direita.";
    return "";
  };

  const executarGeracaoSiteVisual = async () => {
    if (uploadedImages.length === 0) { (window as any).showNotification('Forneça uma imagem de referência estrutural.', 'error'); return; }
    const isMenu = (document.getElementById('checkComMenu') as HTMLInputElement)?.checked ? "INCLUIR BARRA DE NAVEGAÇÃO FIXA NO TOPO." : "SEM BARRA DE NAVEGAÇÃO.";
    
    let promptParts: any[] = [{ text: "Gere o código HTML com Tailwind CSS completo aplicando engenharia reversa nesta imagem estrutural:" }];
    uploadedImages.forEach(img => promptParts.push({ inlineData: { mimeType: img.mimeType, data: img.data } }));
    
    const instrucoesFinais = `Desenvolvedor Front-End Sênior. \n${isMenu} \n${getMegaPromptEstilo()} \n${getMegaPromptHero()} \n${getMegaPromptCores()}`;
  ...
  const executarGeracaoSiteTexto = async () => {
    ...
    if (!content) { (window as any).showNotification('Forneça as informações do projeto (Briefing).', 'error'); return; }
    const isMenu = (document.getElementById('checkComMenuTexto') as HTMLInputElement)?.checked ? "INCLUIR BARRA DE NAVEGAÇÃO FIXA NO TOPO." : "SEM BARRA DE NAVEGAÇÃO.";
    const instrucoesFinais = `Especialista em Marketing e Desenvolvedor Front-End. Crie uma Landing Page completa do zero com base nestas informações: \n${isMenu} \n${getMegaPromptEstilo()} \n${getMegaPromptHero()} \n${getMegaPromptCores()}`;
  ...
  function processarRespostaDOM(data: any) {
      ...
      (window as any).showNotification(`Interface gerada com sucesso.`, 'success');
  ...
  const gerarNovaImagemIAAutomatica = async (isBackground = false) => {
      if(!elementoSelecionado) return;
      (window as any).showNotification("Buscando imagem profissional adequada...", "success");
      ...
      } catch(err) { (window as any).showNotification("Falha ao processar a busca de imagem.", "error"); }
  };
  ...
  const deletarSite = async (id: string, slug: string) => {
    if (!confirm(`Tem certeza que deseja excluir o projeto permanentemente?`)) return;
  ...
  useEffect(() => {
    ...
    (window as any).baixarHtmlGerado = () => {
      ...
      a.download = siteEditando ? `${siteEditando.slug}.html` : 'exportacao-do-site.html'; a.click();
    };
    ...
    (window as any).handlePublicarSite = async () => {
      ...
      if (siteEditando) { await supabase.from('sites_gerados').update({ html_content: cleanHtml }).eq('id', siteEditando.id); (window as any).showNotification('Site atualizado com sucesso.', 'success'); return; }
      const nome = prompt('Nome do Site para o Link (URL Exclusiva):'); if (!nome) return; 
      ...
      alert(`Publicação concluída!\nLink Copiado: ${window.location.origin}/${slug}`);
    };
  }, [siteEditando]); 
      ...
      <div className="w-[340px] bg-white border-r border-zinc-200 flex flex-col h-full z-10 flex-shrink-0 shadow-sm">
          ...
          <div className="p-4 border-b border-zinc-200 flex items-center justify-between bg-zinc-50/50">
              <h1 className="text-lg font-black tracking-tight text-zinc-900 flex items-center">
                  ...
                  Builder<span className="text-zinc-400 font-medium">Pro</span>
              </h1>
                      ...
                      {!elementoSelecionado ? (
                          ...
                          <div className="pb-10">
                                  ...
                                  <>
                                      <div className="panel-section">
                                          ...
                                          <div className="flex gap-2">
                                              <button onClick={() => gerarNovaImagemIAAutomatica(false)} className="flex-1 bg-zinc-100 hover:bg-zinc-200 text-zinc-700 text-[10px] font-bold py-1.5 rounded transition"><i className="fas fa-magic mr-1"></i> Gerar com IA</button>
                                              <label className="flex-1 bg-zinc-100 hover:bg-zinc-200 text-zinc-700 text-[10px] font-bold py-1.5 rounded text-center cursor-pointer transition"><i className="fas fa-upload mr-1"></i> Importar Arquivo<input type="file" accept="image/*" className="hidden" onChange={(e) => handleUploadImgElem(e, false)} /></label>
                                          </div>
                                      </div>
                                      <div className="panel-section grid grid-cols-2 gap-4">
                                          <div>
                                              ...
                                              <select onChange={(e) => atualizarElemento('imgFormat', e.target.value)} className="input-standard">
                                                  ...
                                                  <option value="aspect-video">16:9 (Horizontal)</option>
                                                  <option value="aspect-[3/4]">3:4 (Vertical)</option>
                                                  <option value="aspect-square">1:1 (Quadrado)</option>
                                              </select>
                                          </div>
                                          <div>
                                              <label className="input-label">Arredondamento</label>
                                              <select onChange={(e) => atualizarElemento('imgRounded', e.target.value)} className="input-standard">
                                                  <option value="rounded-none">0px (Reto)</option>
                                                  <option value="rounded-md">6px (Suave)</option>
                                                  <option value="rounded-xl">12px (Arredondado)</option>
                                                  <option value="rounded-full">100% (Círculo)</option>
                                              </select>
                                          </div>
                                      </div>
                                  ...
                                  <>
                                      {(elementoSelecionado.tagName === 'a' || elementoSelecionado.tagName === 'button') && (
                                          <div className="panel-section bg-blue-50/30">
                                              <label className="input-label text-blue-700">Link de Destino (HREF)</label>
                                      ...
                                      <div className="panel-section grid grid-cols-2 gap-4">
                                          <div>
                                              <label className="input-label flex justify-between">Tipografia <span>{elementoSelecionado.fontSize}px</span></label>
                                          ...
                                          <div className="flex flex-col gap-2">
                                              <div className="flex justify-between items-center">
                                                  <label className="text-[10px] font-semibold text-zinc-600">Cor de Fundo</label>
                                      ...
                                      <div className="panel-section grid grid-cols-2 gap-4">
                                          <div className="col-span-2">
                                              <label className="input-label">Animações e Efeitos (Hover)</label>
                                              <select onChange={(e) => atualizarElemento('animationClass', e.target.value)} className="input-standard">
                                                  <option value="">Desativado</option>
                                                  <option value="hover:scale-105 transition-transform duration-300">Aumentar +5% (Ao passar o mouse)</option>
                                                  <option value="hover:-translate-y-2 transition-transform duration-300">Levantar (Ao passar o mouse)</option>
                                                  <option value="animate-pulse">Pulsar (Atenção contínua)</option>
                                              </select>
                                          </div>
                                          <div className="col-span-2">
                                              <label className="input-label">Classes Tailwind (Avançado)</label>
                              ...
                              <div className="m-4 bg-zinc-900 rounded-lg p-4 shadow-md text-white">
                                  <label className="text-[10px] font-black uppercase tracking-widest text-zinc-300 mb-3 flex items-center"><i className="fas fa-bolt mr-2 text-yellow-400"></i> Otimização Inteligente</label>
                                  
                                  {elementoSelecionado.tagName !== 'img' && (
                                      <div className="grid grid-cols-2 gap-2 mb-3">
                                          <button onClick={() => otimizarComIA("Reescreva com copy persuasiva de alta conversão, tom profissional e conciso")} className="bg-zinc-800 hover:bg-zinc-700 text-[9px] font-semibold py-2 rounded text-zinc-300 transition">Copy Persuasiva</button>
                                          <button onClick={() => otimizarComIA("Reescreva gerando forte urgência, escassez e apelo à ação imediata")} className="bg-zinc-800 hover:bg-zinc-700 text-[9px] font-semibold py-2 rounded text-zinc-300 transition">Gerar Urgência</button>
                                      </div>
                                  )}
                                  <div className="flex gap-2 relative">
                                      <input type="text" id="ai_prompt_element" placeholder="Digite seu comando livre..." className="w-full bg-zinc-800 border border-zinc-700 text-white text-[11px] rounded-md px-3 py-2 outline-none focus:border-zinc-500 placeholder-zinc-500" />
                      ...
                      <div className="p-4 space-y-5">
                          <div>
                              ...
                              <div className="space-y-3 bg-zinc-50 border border-zinc-200 p-3 rounded-lg">
                                  <div>
                                      <label htmlFor="nichoEstilo" className="input-label">Estilo Visual Base</label>
                                  ...
                                  <div>
                                      <label htmlFor="paletaCores" className="input-label">Sistema de Cores (Paleta)</label>
                                      <select id="paletaCores" value={corSelecionada} onChange={(e) => setCorSelecionada(e.target.value)} className="input-standard">
                                          <option value="auto">Extrair Cores Exatas da Imagem (Automático)</option>
                                          <option value="dark">Modo Escuro (Alto Contraste)</option>
                                          <option value="azul">Azul Corporativo (Confiança)</option>
                                          <option value="verde">Verde Esmeralda (Crescimento/Saúde)</option>
                                          <option value="cinza">Tons de Cinza / Monocromático</option>
                                      ...
                                      {corSelecionada === 'personalizada' && (
                                          <div className="flex gap-3 mt-3 p-3 bg-white rounded border border-zinc-200">
                                              <div className="flex-1"><label className="input-label">Fundo Principal</label><input type="color" id="corFundo" className="w-full h-8 rounded cursor-pointer p-0 border border-zinc-200" defaultValue="#ffffff" /></div>
                                              <div className="flex-1"><label className="input-label">Destaque / Botões</label><input type="color" id="corPrimaria" className="w-full h-8 rounded cursor-pointer p-0 border border-zinc-200" defaultValue="#09090b" /></div>
                                          </div>
                                      )}
                                  </div>
                              </div>
                          ...
                          <div>
                              ...
                              <div className="space-y-3 bg-zinc-50 border border-zinc-200 p-3 rounded-lg">
                                  <div>
                                      <label htmlFor="heroLayout" className="input-label">Cabeçalho (Primeira Dobra)</label>
                                      <select id="heroLayout" className="input-standard">
                                          <option value="auto">Estrutura Inteligente (Pela Imagem)</option>
                                          <option value="center">Centralizado (Foco total no Texto)</option>
                                          <option value="split">Dividido (Texto Esquerda, Imagem Direita)</option>
                                      </select>
                                  </div>
                                  <label className="flex items-center gap-2 cursor-pointer mt-1">
                                      ...
                                      <span className="text-xs font-semibold text-zinc-700">Incluir Barra de Navegação Fixa</span>
                                  </label>
                              </div>
                          </div>
                          ...
                          {abaAtiva === 'visual' ? (
                              <div>
                                  <h3 className="text-[11px] font-black uppercase text-zinc-800 mb-3 tracking-wide flex items-center"><span className="w-1.5 h-1.5 rounded-full bg-blue-500 mr-2"></span>3. Clonagem Visual por Imagem</h3>
                                  <div className="bg-white border-2 border-dashed border-zinc-200 hover:border-blue-400 hover:bg-blue-50/20 transition-colors rounded-xl p-6 flex flex-col items-center justify-center text-center cursor-pointer" onClick={() => document.getElementById('imageUploadInput')?.click()}>
                                      ...
                                      <p className="text-[10px] font-medium text-zinc-400 mt-1">Arraste ou Ctrl+V para colar.</p>
                                  </div>
                                  ...
                                  <button onClick={executarGeracaoSiteVisual} className="w-full mt-4 bg-zinc-900 hover:bg-zinc-800 text-white font-bold py-3 rounded-lg transition-colors text-xs flex items-center justify-center gap-2">
                                      <i className="fas fa-code"></i> Construir Interface HTML
                                  </button>
                              </div>
                          ) : (
                              <div>
                                  <h3 className="text-[11px] font-black uppercase text-zinc-800 mb-3 tracking-wide flex items-center"><span className="w-1.5 h-1.5 rounded-full bg-purple-500 mr-2"></span>3. Geração via Texto (Briefing)</h3>
                                  ...
                                  <button onClick={executarGeracaoSiteTexto} className="w-full mt-4 bg-zinc-900 hover:bg-zinc-800 text-white font-bold py-3 rounded-lg transition-colors text-xs flex items-center justify-center gap-2">
                                      <i className="fas fa-code"></i> Construir Site Completo
                                  </button>
                              </div>
                          )}
                      </div>
          ...
          <div className="bg-white border-b border-zinc-200 flex justify-between items-center px-4 md:px-6 h-14 shadow-sm z-10">
              <div className="flex items-center gap-3">
                  <div className="flex bg-zinc-100 p-0.5 rounded-md border border-zinc-200">
                      <button id="tabPreview" onClick={() => (window as any).mudarSeparador('preview')} className="px-4 py-1.5 rounded-md font-semibold text-[11px] bg-white text-zinc-900 shadow-sm transition">Visualização</button>
              ...
              <div className="flex items-center gap-3">
                  ...
                  <button onClick={() => (window as any).baixarHtmlGerado()} className="text-zinc-600 hover:text-zinc-900 text-[11px] px-2 py-1.5 transition" title="Exportar HTML"><i className="fas fa-download"></i></button>
                  <button onClick={() => (window as any).copiarCodigo()} className="text-zinc-600 hover:text-zinc-900 text-[11px] px-2 py-1.5 transition mr-2" title="Copiar Código"><i className="fas fa-copy"></i></button>
                  ...
                  {siteEditando ? (
                      <div className="flex gap-2">
                          ...
                          <button onClick={() => (window as any).handlePublicarSite()} className="px-4 py-1.5 bg-zinc-900 hover:bg-black text-white font-bold text-[11px] rounded-md transition">Atualizar Site</button>
                      </div>
                  ) : (
          ...
          <div className="flex-grow relative bg-zinc-100 p-0 md:p-6 lg:p-8 overflow-hidden flex justify-center">
              {modoInspetor && (
                  <div className="absolute top-4 left-1/2 -translate-x-1/2 z-50 bg-zinc-900 text-white px-6 py-2 rounded-full shadow-lg font-bold text-[10px] uppercase tracking-widest flex items-center gap-2 border border-zinc-700 animate-pulse pointer-events-none">
                      <i className="fas fa-crosshairs text-blue-400"></i> Selecione um elemento no Site para Inspecionar
                  </div>
              )}
              ...
              <div className={`w-full h-full max-w-[1440px] bg-white mx-auto shadow-2xl relative flex flex-col overflow-hidden transition-all duration-300 ${modoInspetor ? 'ring-4 ring-blue-500/20 rounded-lg' : 'rounded-none md:rounded-xl border border-zinc-200'}`}>
                  {modoInspetor && (
                      <div className="h-6 w-full bg-zinc-100 border-b border-zinc-200 flex items-center px-4 gap-1.5">
                          ...
                          <div className="mx-auto bg-white border border-zinc-200 text-[9px] text-zinc-400 px-12 py-0.5 rounded font-mono">Tela: Desktop 1440px</div>
                      </div>
                  )}
        ...
        <div className="fixed inset-0 bg-zinc-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl w-full max-w-4xl max-h-[85vh] flex flex-col overflow-hidden shadow-2xl border border-zinc-200">
            ...
            <div className="p-6 flex-1 overflow-y-auto custom-scrollbar bg-white">
              {carregandoSites ? <p className="text-center text-sm font-medium text-zinc-400 py-12">Carregando projetos...</p> : listaSites.length === 0 ? <p className="text-center text-sm font-medium text-zinc-400 py-12">Nenhum projeto encontrado no servidor.</p> : (
                  <>
                        ...
                        return (
                          <div key={site.id} className="border border-zinc-200 rounded-lg p-4 hover:border-zinc-300 hover:shadow-md transition bg-white flex flex-col">
                            ...
                            <div className="flex justify-between items-center mt-auto pt-4 border-t border-zinc-100">
                              <a href={`/${site.slug}`} target="_blank" rel="noreferrer" className="text-[10px] font-bold uppercase text-zinc-500 hover:text-zinc-900 transition flex items-center"><i className="fas fa-external-link-alt mr-1.5"></i> Acessar Site</a>
                              <div className="flex gap-2">
                                <button onClick={() => editarSite(site)} className="px-3 py-1.5 bg-zinc-900 hover:bg-black text-white text-[10px] font-bold rounded transition">Abrir no Editor</button>
