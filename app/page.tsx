...
export default function Home() {
  ...
  const [modificacaoGlobal, setModificacaoGlobal] = useState(false); // <--- ESTADO DO MODIFICADOR GLOBAL
...
<body class="antialiased text-slate-800 bg-white" style="font-family: '${fontFamily}', sans-serif;">
  ...
  // =========================================================================
  // FUNÇÃO HÍBRIDA (SUPER BLINDADA) - LIDA COM GLOBAL OU LOCAL
  // =========================================================================
...
6. IMAGENS: Se precisar criar uma nova seção com imagens, use a regra <img data-tema="palavras,chave" /> para o sistema hidratar depois.`;
              ...
              if(resData && resData.html) {
                  ...
                  // Hidrata as imagens para o site inteiro caso a IA crie novas
          ...
          // --- MODO LOCAL (APENAS ELEMENTO SELECIONADO) ---
...
6. IMAGENS: Se precisar criar uma nova seção com imagens, use a regra <img data-tema="palavras,chave" /> para o sistema hidratar depois.`;
        ...
        if (resData && resData.html) {
            const cleanHtml = resData.html.replace(/```html/gi, '').replace(/```/g, '').trim();
            // Passa pelo hidratador caso o usuário peça novas imagens
            const htmlHidratado = await preencherImagensAutomaticamente(cleanHtml);
            processarRespostaDOM({ html: htmlHidratado }); 
