...
export default function Home() {
  ...
  const buscarImagemGratuita = async (tema: string, w = 1000, h = 800) => {
      ...
      if (activeApis.unsplash && unsplashKey && !finalUrl) {
          try {
              const res = await fetch(`[https://api.unsplash.com/search/photos?query=$](https://api.unsplash.com/search/photos?query=$){termo}&page=${pagina}&per_page=15&client_id=${unsplashKey}`);
              if (res.ok) {
      ...
      if (activeApis.pexels && pexelsKey && !finalUrl) {
          try {
              const res = await fetch(`[https://api.pexels.com/v1/search?query=$](https://api.pexels.com/v1/search?query=$){termo}&page=${pagina}&per_page=15`, {
      ...
      if (activeApis.pixabay && pixabayKey && !finalUrl) {
          try {
              const res = await fetch(`[https://pixabay.com/api/?key=$](https://pixabay.com/api/?key=$){pixabayKey}&q=${termo}&image_type=photo&orientation=horizontal&per_page=15`);
              if (res.ok) {
      ...
      if (!finalUrl) {
          finalUrl = `[https://placehold.co/$](https://placehold.co/$){w}x${h}/e2e8f0/475569?text=` + termo;
      }
        ...
        if (apiKey && apiKey.length > 10) {
            ...
            for (const modelName of modelosDeTexto) {
                ...
                try {
                    ...
                    // 🛡️ BLINDAGEM: Lê como texto primeiro para evitar o erro "Unexpected token '<'"
                    const responseText = await response.text();
                    let data;
                    try {
                        data = JSON.parse(responseText);
                    } catch (e) {
                        throw new Error(`Google retornou instabilidade no servidor (Status ${response.status}). Tentando próximo modelo...`);
                    }
